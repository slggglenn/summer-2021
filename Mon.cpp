...
Mon::Mon(float new_x, float new_y, std::string text_loc) : Character(new_x, new_y, text_loc)
{
	...
	hitbox_ = { Character::getSprite().getPosition().x, Character::getSprite().getPosition().y, Character::getSprite().getGlobalBounds().width
					 , 
				      Character::getSprite().getGlobalBounds().height
					  }; // how to set location and ave follow?
...
void Mon::updateHitbox()
{
	...
	hitbox_.left = Character::getSprite().getPosition().x;//getGlobalBounds().left;
	hitbox_.top = Character::getSprite().getPosition().y;
}
